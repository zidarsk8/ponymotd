...
def syscall(callstr):
    return subprocess.check_output(callstr, shell=True).decode(encoding='UTF-8').strip()


ipArray = {i.strip().split(" ")[-1]:i.strip().split(" ")[1] for i in syscall("ip addr | grep 'scope global'").splitlines()}
...
cpuinfo = syscall("cat /proc/cpuinfo | grep 'model name\s:' | head -n 1 | sed 's/model name\s: //g'")

meminfo = syscall("cat /proc/meminfo").splitlines()

memdict = {line.split(":")[0] : int(line.replace(" kB","").split(" ")[-1])//1024 for line in meminfo}

memall = memdict["MemTotal"]
memfree = memdict["MemFree"]*100//memdict["MemTotal"]

lastlogin = syscall("last %s | grep -v 'still logged in' | head -n1" % (username) )
...
if "Arch" in release:
    ...
    upgrades = syscall("/usr/lib/update-notifier/apt-check 2>&1 | cut -d ';' -f 1")
...
data.append(  "{p}%s{w} %s {p}%s" % ("+"*(np+1), "User Data", "+"*(np+1)) )
